...
if st.button("Run Test") and uploaded_file:
    ...
    # Flag unsafe words for live preview
    preview_rows = []
    ...
    # Convert back to CSV as a file-like object (fix for NameError)
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerows(cleaned_rows)
    output.seek(0)
    cleaned_csv = output  # now a proper file-like object

    # Send to backend (inside the button block!)
    url = "https://charactergaurd-1.onrender.com/run/production"
    files = {
        "conversations": ("data.csv", cleaned_csv, "text/csv")
    }
